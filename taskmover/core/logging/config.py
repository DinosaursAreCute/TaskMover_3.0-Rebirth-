...
# Import LogLevel from interfaces to maintain consistency
...
class ConfigurationLoader:
    ...
    def _create_config_from_dict(self, data: dict[str, Any]) -> LoggingConfig:
        ...
        for component, level_str in data.get("components", {}).items():
            try:
                components[component] = self._parse_level(level_str)
            except ValueError:
                print(
                    f"Warning: Invalid log level '{level_str}' for component '{component}'"
                )
    ...
    def _apply_environment_overrides(self):
        ...
        if env_level := os.getenv("TASKMOVER_LOG_LEVEL"):
            try:
                self._config.level = self._parse_level(env_level)
            except ValueError:
        ...
        if env_colors := os.getenv("TASKMOVER_LOG_COLORS"):
            self._config.console.colors = env_colors.lower() in (
                "true",
                "1",
                "yes",
                "on",
    ...
    def _validate_config(self):
        ...
        if not self._validate_size_format(self._config.file.rotation.max_size):
            print(
                f"Warning: Invalid max_size format: {self._config.file.rotation.max_size}"
            )
    ...
    def _parse_level(self, level_str: str) -> LogLevel:
        ...
        try:
            ...
            # Map string names to LogLevel enum members
            level_map = {
            ...
            return level_map.get(level_name, LogLevel.INFO)
        except Exception:
    ...
    def create_default_config_file(self, path: Path | None = None) -> Path:
        ...
        default_config = {
            "logging": {
                "level": "INFO",
                "console": {"enabled": True, "colors": True, "format": "compact"},
                "file": {
                    "enabled": True,
...
def parse_size_to_bytes(size_str: str) -> int:
    ...
    match = re.match(
        r"^(\d+(?:\.\d+)?)\s*([KMGT]?B)$", size_str.upper().replace(" ", "")
    )
    if not match:
